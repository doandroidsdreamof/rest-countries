...
const res = await fetch(`https://restcountries.com/v3.1/name/${getName}`)
 ...
 if(filterCountries.name.toLowerCase().charAt(i) !== getName.toLowerCase().charAt(i)) return
  ...
  return (
    ...
    <nav  className={load ? " relative   " : `${dark ? 'bg-dark-blue border-b-very-dark-blue border-0' : '  border-b-gray-300 bg-white border-2'} relative   animate-pulse`}>
    ...
    <div class="relative">
        ...
        <input type="search"  className={`${dark ? 'text-white bg-dark-blue  border-gray-500' : 'text-gray-900 bg-gray-50  border-gray-300'}  HiddenWhenNavigate animation block md:w-1/3 md:left-14 md:relative p-4 pl-10 w-full  text-sm  rounded-lg border focus:ring-blue-500`} placeholder="Search a country..." onChange={(getName) =>searchCountries(getName.target.value) }/>
    </div>
      <select className={`${dark ? 'bg-dark-blue text-white' : ' bg-white'}   HiddenWhenNavigateOpt focus:ring-blue-500 focus:ring-2 mr-auto my-5 md:-translate-y-4 md:right-24 p-3 font-NunitoSans block md:absolute shadow-md rounded-md cursor-pointer`} onChange={(getRegion) =>filterRegions(getRegion.target.value) }>
    ...
    <Route  path="/"
           ...
           {countries.slice(0,`${show}`).map((country,index) =>{
             ...
             <img className={!load ? 'opacity-0' : "h-1/2 object-cover w-full rounded-t-md"} 
               ...
               <div className={!load ? "opacity-0 w-full p-4" : `w-full p-4`}>
               <h1  className={`${dark ? 'text-white' : 'text-black' } text-xl pl-1 font-sixHundred`}> {country.name.common}</h1>
               <h3  className={`${dark ? 'text-white' : 'text-black' } text-base pl-1  pt-1`}><strong>Population:</strong> {new Intl.NumberFormat().format(country.population)}</h3>
               <h3  className={`${dark ? 'text-white' : 'text-black' } text-base  pl-1 pt-1 `}><strong>Region:</strong> {country.region}</h3>
               {/*if capital is not found return string*/}
               <h3  className={`${dark ? 'text-white' : 'text-black' } text-base pl-1 pt-1 md:pb-5`  }><strong>Capital:</strong> {country.capital ? country.capital : "None"}</h3>               
               </div>          
           </div>
           </main>
             );
